...
async function init() {
  try {
    ...
    showBanner(
      "Could not reach ClickHouse. Check your <code>.env</code> connection details and that the <code>forex</code> table is loaded.<br>Details: " +
        e.message
    );
    return;
  }
...
async function load() {
  ...
  if (start > end) {
    showBanner("The <b>From</b> date is after the <b>To</b> date.");
    return;
  }
  hideBanner();
...
function updateKpis(k) {
  ...
  el("kpi-range").innerHTML =
    k.low == null ? "—" : fmtPrice(k.low) + " <small>/</small> " + fmtPrice(k.high);
