...
Work on this workshop happens on the `build-workshop-v1` branch. The playbook also runs
fully self-paced — no instructor needed (see the learner track's "Running this workshop
self-paced" and "Troubleshooting" pages).
...
Editable draw.io versions of this diagram (plus data-flow and module-flow variants) live
in `docs/diagrams/` — the `.drawio.svg` exports embed the diagram XML, so opening them in
draw.io recovers the editable original.
...
flowchart LR
  ...
  M06 --> M07["07 Chat + Langfuse<br/>15 min"]
  M07 --> M08["08 Break and fix<br/>20 min"]
  M08 --> M09["09 Wrap-up<br/>take it home<br/>10 min"]
  M08 -.->|pick one or more| F["fault/01-map-not-loading<br/>fault/02-zone-stats-500<br/>fault/03-slow-dashboard"]
```

Modules 00-07 and 09 stay on `build-workshop-v1`; the only branch switches are the
fault branches in module 08.
...
### Observability and the break-and-fix finale
...
flowchart LR
  ...
  FAULT["module 08: git checkout fault/*<br/>a realistic bug ships"] --> APP
...
| `playbook/` | The published follow-along playbook (Next.js + Fumadocs; dual learner/instructor tracks plus self-paced and troubleshooting pages; deploys to demohouse.cloud/workshop). Requires Node >= 22.12 to build. |
| `docs/` | `WORKSHOP_PLAN_V2.md` (architecture, agenda, decisions, verification gates) and `diagrams/` (editable draw.io sources + SVG exports). |
...
diagnosis paths and fixes live in the playbook's instructor track (module 08) and are
