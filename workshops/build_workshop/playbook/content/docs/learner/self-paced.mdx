...
<Callout type="warn" title="One page to hold back: the module 08 answer key">
  The instructor page for [08 Break and fix](/docs/instructor/08-break-and-fix) contains
  the full fault answer key - the exact symptom, signal, and fix for each injected fault.
  Diagnosing the fault yourself is the entire point of module 08, so attempt it from the
