...
![LibreChat Agent Builder "Create New Agent" form with a name and the SRE instructions pasted into the Instructions box, and an OpenAI model selected](/screenshots/06b-agent-create.png)

Scroll to **MCP Servers** → **Add MCP Server Tools** and add both servers — `clickstack` and
`github`:

![The MCP Server Tools picker showing the clickstack and github MCP servers, each with an Add button](/screenshots/06b-agent-add-mcp.png)
...
![The agent's MCP Servers section listing both clickstack and github](/screenshots/06b-agent-mcp-attached.png)
...
![The saved agent answering "what can you do?", describing symptom confirmation, querying ClickStack telemetry, localizing failures, and reading source from ClickHouse/ClickHouse_Demos](/screenshots/06b-agent-ready.png)
