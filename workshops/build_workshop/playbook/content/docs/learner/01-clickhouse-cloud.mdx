...
Module 00 loaded `.env.workshop` into this terminal, so this command uses those values
directly. If you opened a new terminal, reload them first with
`set -a; source ./.env.workshop; set +a`. The client reads `CLICKHOUSE_PASSWORD` from the
exported environment, keeping the secret out of process arguments and leaving standard
input available for the copyable SQL blocks below.
