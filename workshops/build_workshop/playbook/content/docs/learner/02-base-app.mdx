...
The screenshot below shows the Ops dashboard with the **Use sample window** button
applied, so you can see the layout populated with seeded data — by default it opens on the
live (today) window, which stays empty until you start CDC in module 03.

![The NYC Taxi Ops (Live) dashboard: the Filters card set to the July 2022 sample window, the "Act 1 — What's happening now?" chart of trips, fare, and p50/p95 duration, and the Top pickup zones bar chart](/screenshots/02-ops-dashboard.png)
...
![The Historical Metrics dashboard over a full-year 2022 window: the Filters card, the KPI Timeseries chart of trips by month, and the day-of-week by hour Seasonality heatmap](/screenshots/02-historical-dashboard.png)
...
Module 03 adds the live path:
pg-trip-writer  ->  managed Postgres  ->  ClickPipe  ->  ClickHouse Cloud  ->  dashboard
```
...
You have a mental model of the app: two dashboards, a FastAPI API querying ClickHouse,
a Postgres CDC source that is added next, and a chat panel waiting to be connected. Every
later module changes one part of this picture.

## End state

You understand the running app. Continue to
[03 Managed Postgres CDC](/docs/learner/03-realtime-cdc) to stream live rows into the Ops
dashboard.
