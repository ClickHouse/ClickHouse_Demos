...
- **Fix** - just retry, or wait about 30 seconds. This is not a fault. It also matters in
  [08 Break and fix](/docs/learner/08-break-and-fix): a waking service makes the fault 03
...
- **Why** - expected before you complete the browser OAuth flow; the endpoint is
  authenticated.
- **Fix** - add the server to your agent, then run the OAuth flow (for example `/mcp` in
  Claude Code, or `claude mcp login <name>`) and authorize in the browser. Also confirm the
  **Connect with MCP** toggle is on for your service.
