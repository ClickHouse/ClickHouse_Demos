...
- Run one fault at a time. Teach in the order 02 -> 03 -> 01 (clearest telemetry signal
  first, most subtle last); `git checkout build-workshop-v1` and rebuild between faults.
...
```bash
git checkout build-workshop-v1 && docker compose --env-file .env.workshop \
...
- Fault symptom not yet visible because the stack was just brought up — or, for fault 03,
  because too little historical data is loaded (confirm the module 01 seed ran).
...
- `git checkout build-workshop-v1` restores the complete app for anyone stranded.
