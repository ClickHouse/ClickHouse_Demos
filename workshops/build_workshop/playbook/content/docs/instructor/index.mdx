...
their answer key (and to the module 08 fault answer key only as a last resort), so keep
...
| 05 | ClickStack | 15 min | Telemetry needs time to accumulate before module 08 |
| 06 | AI SRE | 15 min | MCP OAuth is the fragile moment; pre-stage config |
| 07 | Chat and Langfuse | 15 min | Keys already set in module 00; restart plus trace tour |
| 08 | Break and fix | 20 min | The finale; hard-stop management matters |
...
<Callout title="Tune the timing during rehearsal">
  ...
  and set a hard-pivot time for the finale (module 08) so the wrap-up is never squeezed.
</Callout>
