...
echo "      Step 2    Create the empty tables  →  SQL console + scripts/00_seed_zones.sql + dbt run"
echo "      Step 3    Migrate data  →  scripts/02_migrate_trips.py"
echo ""
echo "    Module 04 — Rebuild the dbt pipeline"
echo "      Step 1    Populate analytics layer  →  dbt run"
echo "      Step 2    Create dictionary  →  scripts/04_create_dictionary.sql"
echo ""
echo "    Module 05 — Benchmark and cutover"
echo "      Step 1    Set up Superset dashboards  →  superset/add_clickhouse_connection.sh"
echo "      Step 2    Run benchmark  →  scripts/run_benchmark.sh"
echo "      Step 3    Cutover (optional)  →  scripts/03_cutover.sh"
echo "      Step 4    Verify data parity  →  scripts/01_verify_migration.sh"
echo ""
echo -e "${GREEN}${BOLD}════════════════════════════════════════════════════════════${RESET}"
